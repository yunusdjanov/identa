...
import {
    APPOINTMENT_COMPACT_NAME_UI_LIMIT,
    APPOINTMENT_MODAL_NAME_UI_LIMIT,
    APPOINTMENT_MODAL_REASON_UI_LIMIT,
    APPOINTMENT_NAME_UI_LIMIT,
    APPOINTMENT_REASON_UI_LIMIT,
    getAppointmentBorderClass,
    getAppointmentCardClass,
    getAppointmentIdentityPayload,
    getAppointmentStatusBadgeClass,
    getAvailableAppointmentStartTimes,
    getDurationMinutes,
    getSlotOrderPriority,
    getWeekStart,
    getWeekViewCompactVisibleAppointments,
    isNonBlockingAppointmentStatus,
    noopSubscribe,
    parseStatusFilter,
    subscribeViewportChanges,
    WEEK_VIEW_COMPACT_CARD_HEIGHT_CLASSES,
    WEEK_VIEW_COMPACT_LIST_HEIGHT_CLASSES,
    WEEK_VIEW_MIN_VISIBLE_APPOINTMENTS,
    WEEK_VIEW_STACKED_CARD_HEIGHT_CLASS,
    WEEK_VIEW_STACKED_LIST_HEIGHT_CLASS,
    WEEK_VIEW_STACKED_VISIBLE_APPOINTMENTS,
    type AppointmentRow,
    type AppointmentsWorkspaceMode,
    type AppointmentStatus,
    type WeekInlineEditFormData,
    extractReason,
} from './appointments-workspace.helpers';
...
interface AppointmentsWorkspaceProps {
    mode?: AppointmentsWorkspaceMode;
}

export function AppointmentsWorkspace({ mode = 'appointments' }: AppointmentsWorkspaceProps) {
    const { t, locale } = useI18n();
    const queryClient = useQueryClient();
    const isDashboardMode = mode === 'dashboard';
    const routePath = isDashboardMode ? '/dashboard' : '/appointments';
    const weekViewCompactVisibleAppointments = useSyncExternalStore(
        subscribeViewportChanges,
        getWeekViewCompactVisibleAppointments,
        () => WEEK_VIEW_MIN_VISIBLE_APPOINTMENTS
    );
    ...
    const [isQuickPatientDialogOpen, setIsQuickPatientDialogOpen] = useState(false);
    ...
    const handleQuickPatientDialogOpenChange = (open: boolean) => {
        if (open && !canManagePatients) {
            denyPermission();
            return;
        }

        setIsQuickPatientDialogOpen(open);
    };
    ...
    const appointmentsQuery = useQuery({
        queryKey: queryKeys.appointments.list(visibleRange.startDate, visibleRange.endDate),
    ...
    const configurationQuery = useQuery({
        queryKey: queryKeys.appointments.configuration(),
    ...
    const dayTimeSlotColumns = useMemo(() => {
        const midpoint = Math.ceil(timeSlots.length / 2);
        return [timeSlots.slice(0, midpoint), timeSlots.slice(midpoint)];
    }, [timeSlots]);
    ...
    const renderWeekDayCard = (
        ...
        const compactCardHeightClass =
            WEEK_VIEW_COMPACT_CARD_HEIGHT_CLASSES[weekViewCompactVisibleAppointments]
            ?? WEEK_VIEW_COMPACT_CARD_HEIGHT_CLASSES[WEEK_VIEW_MIN_VISIBLE_APPOINTMENTS];
        const compactListHeightClass =
            WEEK_VIEW_COMPACT_LIST_HEIGHT_CLASSES[weekViewCompactVisibleAppointments]
            ?? WEEK_VIEW_COMPACT_LIST_HEIGHT_CLASSES[WEEK_VIEW_MIN_VISIBLE_APPOINTMENTS];
    ...
    const replaceAppointmentsUrl = (updates: Record<string, string | null>) => {
        ...
        window.history.replaceState(window.history.state, '', nextSearch ? `${routePath}?${nextSearch}` : routePath);
    };
    ...
    // Background refetch failures must not replace usable cached data with a
    // full-page error. This is especially important for auth/me: AppLayout may
    // already have a valid user while a transient focus refetch fails.
    //
    // Practice configuration is an optional enhancement. The workspace has
    // safe default hours/duration, so a configuration outage must not make the
    // appointment calendar unavailable.
    const fatalLoadError = currentUserQuery.data === undefined && currentUserQuery.isError
        ? currentUserQuery.error
        : appointmentsQuery.data === undefined && appointmentsQuery.isError
        ...
        <div className="space-y-5 lg:space-y-6">
            ...
            {isDashboardMode ? (
                <PageHeader
                    ...
                    description={t('dashboard.planner.subtitle')}
                    actions={(
                        <>
                            {canManageAppointments ? (
                                <Button
                            ...
                            {canManagePatients ? (
                                <Button
                                    className="w-full sm:w-auto"
                                    onClick={() => setIsQuickPatientDialogOpen(true)}
                                >
            ...
            {view === 'day' ? (
                        ...
                        <div className="grid gap-x-6 gap-y-1 xl:grid-cols-2">
                            {dayTimeSlotColumns.map((column, columnIndex) => (
                                <div
                                    key={`day-slot-column-${columnIndex}`}
                                    data-testid={`day-slot-column-${columnIndex}`}
                                    className="space-y-1"
                                >
                                    {column.map((time) => {
            ...
            {isQuickPatientDialogOpen ? (
                <AddPatientDialog
                    open={isQuickPatientDialogOpen}
                    onOpenChange={handleQuickPatientDialogOpenChange}
                />
            ) : null}
            <ConfirmActionDialog
